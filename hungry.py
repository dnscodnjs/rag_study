        ...
        for item in test_data:
            ...
            for i, section in enumerate(structure):
                ...
                metadata = {
                    ...
                    "image_paths": image_paths  # image_paths는 리스트 또는 문자열일 수 있음
                }
...
# ==========================================
# 위젯에 고유 key 사용
user_prompt = st.chat_input("메시지를 입력하세요", key="user_prompt")

with st.expander("음성 입력 및 이미지 첨부 열기"):
    audio_file = st.audio_input("음성을 녹음하세요.", key="audio_file")
    uploaded_image = st.file_uploader("이미지를 업로드하세요", type=["png", "jpg", "jpeg", "gif"], key="uploaded_image")
...
if audio_file is not None:
    ...
    # 음성 입력은 처리 후 별도로 초기화하거나, widget 자체는 그대로 두고 후속 처리에서 무시합니다.
...
if user_prompt or uploaded_image:
    ...
    # 입력값 초기화 대신 widget 값을 그대로 둡니다.
    # (해당 위젯은 사용자가 직접 수정할 수 있습니다.)
